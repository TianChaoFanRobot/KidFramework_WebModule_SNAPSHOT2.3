...
public class IocHelper {
	...
	static
	{
		...
		//TODO TCF 属性参数 ParameterName:ParameterValue
			...
			if(beanMap!=null)
			{
				for(Map.Entry<Class<?>,Object> beanEntry:beanMap.entrySet())
				{
					...
					if(targetClass!=null && targetInstance!=null)
					{
						...
						if(fields!=null && fields.length>0)
						{
							for(Field field:fields)
							{
								...
								if(injectFieldFilter(field))
								{
									...
									{
										//TODO TCF 开始IOC/DI依赖注入，给目标实例目标属性注入属性值
										if(fieldMap!=null)
										{
											for(Map.Entry<String,Map<String,Object>> fieldEntry:fieldMap.entrySet())
											{
												...
												//TODO TCF 需要注入的属性所在类simpleName必须和当前加载的属性所在类simpleName相同
												if(StringUtils.isNotEmpty(classSimpleName) && classSimpleName.equals(targetClass.getSimpleName()))
												{
													...
													if(paramMap!=null)
													{
														...
														{
															//TODO TCF 需要注入的属性名 FieldName
															String injectFieldName=paramEntry.getKey();
															
															//TODO TCF 需要注入的属性值 FieldValue
															Object injectFieldValue=paramEntry.getValue();
															
															//TODO TCF 需要注入的属性名必须和当前加载的属性名相同才能注入属性值到该属性
						...
						//TODO TCF 以下条件可以去掉，在这里的目的只是为了测试AOP动态代理增强处理方法动态织入执行效果
